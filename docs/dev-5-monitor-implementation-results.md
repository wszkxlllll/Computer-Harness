...
- action signature 使用本地 SHA-256 opaque hash；`click/double_click/right_click/scroll/drag/wait` 只使用规范化结构，`type` 只使用 text length，`keypress` 只使用数量与 key length。
...
结果：focused Vitest 1 个文件、7 项测试通过；模块隔离 TypeScript 检查退出码 `0`。测试覆盖正常重复不直接 stop、A-B-A 分区、重复 refusal、Plan/Memory churn、无 Plan 短任务、缺图 unknown、geometry 分区、bounded eviction/run reset、unknown outcome 与 typed text 不落状态/输出。
