...
状态：代码与离线自审完成，Sol 已有限放行。本批从 `fa89f24` 创建 `codex/dev2-tui-preview`，只覆盖最小受控 TUI、提交后事件订阅和同进程应用会话；本 worker 尚未调用真实 API、桌面或 VM，也不把 Fake 交互当作 CUA 实机通过。
...
- worker_ci 的实机证据只代表 Windows + CUA 0.22.2 + 专用 fixture 的窄路径，不能外推通用 focus/AX 或完整 TUI Run 已放行；其独立记录的 winpty no-goal home 已验证中文/resize/ESC→Q/Ctrl-C 与 cursor restore，本 worker 仅修复了 Escape `undefined` 输入边界。完整 model Run、通用 focus 和真实业务动作仍未验证。
