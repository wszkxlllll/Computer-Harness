...
| HEAD before final docs-format cleanup commit | `65151396bef8b53ce1e15ad81a558de59b1a05b5` (`docs: record DEV-2 remote push readiness`) |
| baseline | `origin/main` = `bc72ee543197510fce8d3943b0251d2fb51d33bb` |
| ancestry before final docs-format cleanup commit | `origin/main...HEAD` left/right = `0/18`；当前线性领先 18 commits |
| remote head | `git ls-remote --heads origin codex/dev2-tui-preview` 当前无结果，尚未发布 |
| diff size before final docs-format cleanup commit | 77 changed paths，`10,645 additions / 520 deletions`；早期 16-commit 的 `74 / 10,280 / 297` 仅保留为历史 snapshot |
| identity | 18/18 commits 的 author 与 committer 都是 `wszkxlllll <142909575+wszkxlllll@users.noreply.github.com>` |

README 与 `docs/getting-started.md` 已由另一 worker 提交为 `101936f`；readiness 报告已由 `6515139` 提交。本次格式 cleanup 将只包含两份既有 docs 的行尾空格清理与 readiness 更新，预计 PR 将保留 19 个 commits；最终 path/count 应在该 commit 后重算，不在报告中自引用自身 hash。

## 2. 将要发布的 feature group

`origin/main..HEAD` 是一条连续的 DEV-2 批次历史，未执行 rebase、squash 或 cherry-pick；当前 snapshot 为 18 commits，本次格式 cleanup 后预计为 19 commits。提交主题覆盖：
...
这是一个跨 package 的 feature group，而不是只包含单个小修复的 PR。若保持当前历史，建议一个 PR 保留现有 18 个逻辑 commit 加本次格式 cleanup commit，正文按上述四个层次说明；不建议在发布前重写已审历史。若审阅者要求拆分，应在新 review 计划中明确依赖顺序，而不是临时 cherry-pick 或隐式改变 ancestry。

## 3. 敏感内容与资产检查

对 `origin/main..HEAD` 的 changed paths、当前 HEAD tree 以及当前 18 个 commit 的 diff 做了只读扫描：
...
这项检查不读取 `.env`，也不从 ignored 运行资产恢复或打印秘密。README/Getting Started 与 readiness 报告此前扫描均未发现敏感类别；格式 cleanup 完成后已重做最终 `git diff origin/main..HEAD --check`、文件名和 staged set 检查。
...
补充的范围检查：`git diff --check 101936f^..101936f`、readiness 自身历史 commit 和 `6515139` 均为 `0`；在本次 cleanup 前，完整 `git diff --check origin/main..HEAD` exit `2`，报告 7 个历史 Markdown trailing-whitespace 行，全部集中在 `docs/dev-2-cua-capability-and-extension-research.md`（4 行）和 `docs/dev-2-cua-target-integration-assessment.md`（3 行）的 Markdown 硬换行。本次只移除这 7 处行尾空格并用空白段落保持可读，不改变文字内容；cleanup 完成后的 full-range diff-check 已为 `0`。

## 5. 已知限制与发布前门槛

1. 本次格式 cleanup 已提交，远端 branch 尚不存在；最终已确认 `git status` clean、19-commit count 和 staged 文件范围。
2. 这 18 commits + 本次 cleanup / 77 paths 的 review surface 较大；一个 PR 最能保留当前依赖和提交证据，但需要在正文明确 DEV-2 范围及未完成的通用 focus/AX、跨平台、完整 model Run、真实用户桌面和产品化 target 能力。
...
1. 本次格式 cleanup docs commit 已完成；未覆盖或重写已审 README/Getting Started，且未 rebase/squash 历史。
2. cleanup commit 后已重做 identity、`origin/main..HEAD`、敏感文件/内容、`git diff --check` 和 lockfile 漂移检查，并记录最终 19-commit/path count；不要把初始 16/74 snapshot 当最终计数。
3. 最终 docs 只改变文档且业务树未漂移，已复用本节 Node 24 离线集成结果；cleanup 后完整 range diff-check 为 `0`。
...
* 保留当前线性 19-commit DEV-2 feature group（18 个已有 commit 加本次格式 cleanup docs commit），覆盖 app-runtime/session assembly、persistent TUI/correction/feed/text rendering、CUA doctor 与显式 window-target preview，以及对应 focused probes/docs。
* 本地 Node `v24.19.0` / pnpm `11.19.0` 验证：typecheck 通过；全量 32 files / 320 tests 通过；CLI help、frozen-lockfile offline install、lock drift 通过；README/Getting Started/readiness 自身 diff-check 通过；本次 cleanup 移除 7 个历史 Markdown 行尾空格，使 base-to-head full-range diff-check 归零。
