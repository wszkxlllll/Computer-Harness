...
Runtime 不必依赖额外 Risk Model 才知道一个操作是否具有高副作用。很多风险可以在 Tool 或 Action 层进行结构化标注或根据当前执行语义进行规则判断。

普通低风险操作可以直接执行；具有较明显副作用的操作可以要求用户确认；明显不允许的操作可以直接拒绝。
...
这种机制可以显著减少模型调用次数，使安全增强不会导致 Agent Loop 的整体延迟成倍增长。
